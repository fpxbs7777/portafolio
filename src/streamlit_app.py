...
class manager:
    ...
    def _create_output(self, weights):
        """
        Crea un objeto output con los pesos del portafolio calculados.
        
        Args:
            weights: Array de pesos del portafolio
            
        Returns:
            output: Instancia de la clase output con los resultados del portafolio
        """
        # Calcular retornos del portafolio
        if hasattr(self, 'returns') and self.returns is not None and not self.returns.empty:
            port_returns = (self.returns * weights).sum(axis=1)
        else:
            # Si no hay retornos disponibles, crear un array de ceros
            port_returns = pd.Series(0, index=pd.date_range(end=pd.Timestamp.today(), periods=252, freq='B'))
            
        # Crear instancia de output
        port_output = output(port_returns, self.notional)
        
        # Asignar los pesos al objeto output
        port_output.weights = pd.Series(weights, index=self.returns.columns if hasattr(self, 'returns') and self.returns is not None else None)
        
        # Crear DataFrame con la asignación de activos
        if hasattr(self, 'rics') and self.rics is not None:
            allocation_df = pd.DataFrame({
                'Activo': self.rics,
                'Peso': weights,
                'Monto': np.array(weights) * self.notional
            })
    ...
    def compute_portfolio(self, strategy='max_sharpe', target_return=None):
        ...
        try:
            ...
            if strategy == 'max_sharpe':
                # Para max_sharpe, usamos markowitz sin target_return
                ...
                portfolio_output = self.manager.compute_portfolio(
                    portfolio_type='min-variance-l2',
