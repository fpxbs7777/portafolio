...
{{ ... }}
    ...
    for simbolo, activo in portafolio.items():
        try:
            ...
            valuacion_activo = activo.get('Valuación', 0)
            # Asegurar que la valuación sea un número válido
            if valuacion_activo is None:
                valuacion_activo = 0
            try:
                valuacion_activo = float(valuacion_activo)
            except (ValueError, TypeError):
                valuacion_activo = 0
            peso = valuacion_activo / valor_total if valor_total > 0 else 0
    ...
    retorno_esperado_anual = sum(
        ...
        if m['peso'] is not None and m['retorno_medio'] is not None
    )
        ...
        if len(retornos_diarios) > 1:
            ...
            if len(df_retornos) < 5:  # Mínimo de datos para correlación confiable
                ...
                volatilidad_portafolio = sum(
                    ...
                    for m in metricas_activos.values()
                    if m['peso'] is not None and m['volatilidad'] is not None
                )
            else:
    ...
    for _ in range(1000):  # Simulación Monte Carlo simple
        retorno_simulado = 0
        for m in metricas_activos.values():
            # Verificar que todos los valores necesarios sean válidos
            if (m['peso'] is not None and m['retorno_medio'] is not None and 
                m['volatilidad'] is not None):
                ...
                for fecha in precios_recientes.index:
                    ...
                    for activo, peso in zip(activos, pesos):
                        if activo in precios_recientes.columns:
                            # Verificar que peso sea un número válido
                            if peso is not None:
                                try:
                                    peso_valido = float(peso)
                                    valor_fecha += peso_valido * precios_recientes.loc[fecha, activo]
                                except (ValueError, TypeError):
                                    continue
