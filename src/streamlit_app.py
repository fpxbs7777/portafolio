...
def mostrar_analisis_portafolio():
    ...
    with tab5:
        id_cliente = cliente.get('numeroCliente', cliente.get('id'))
        mostrar_test_inversor(token_acceso, id_cliente)
...
class PortfolioManager:
    ...
    def compute_portfolio(self, strategy='markowitz', target_return=None):
        """
        Computa la optimización del portafolio usando el módulo optimizer
        """
...
def obtener_test_inversor(token_portador):
    """
    Obtiene el formulario del test del inversor
    """
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            st.error(f"Error obteniendo test del inversor: {response.status_code}")
            return None
    except Exception as e:
        st.error(f"Error en la solicitud del test: {str(e)}")
        return None

def enviar_test_inversor(token_portador, id_cliente, respuestas):
    """
    Envía las respuestas del test del inversor
    """
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            st.error(f"Error enviando test del inversor: {response.status_code}")
            return None
    except Exception as e:
        st.error(f"Error en la solicitud del test: {str(e)}")
        return None

def mostrar_test_inversor(token_portador, id_cliente):
    """
    Muestra y procesa el test del inversor
    """
    ...
    if test_data:
        with st.form("test_inversor_form"):
            ...
            instrumentos_seleccionados = st.multiselect(
                "¿En qué instrumentos ha invertido anteriormente?",
                options=[(i['id'], i['nombre']) for i in instrumentos],
            ...
            for nivel in niveles:
                opcion = st.selectbox(
                    f"Nivel de conocimiento en {nivel['nombre']}",
                    options=[(o['id'], o['nombre']) for o in nivel['opciones']],
                    format_func=lambda x: x[1]
                )
                niveles_conocimiento.append(opcion[0] if opcion else 0)
            
            # Otros campos del test
            col1, col2 = st.columns(2)
            
            with col1:
                plazo = st.selectbox(
                    "Plazo de inversión",
                    options=[(p['id'], p['nombre']) for p in test_data.get('plazosInversion', {}).get('plazos', [])]
                )
                
                edad = st.selectbox(
                    "Rango de edad",
                    options=[(e['id'], e['nombre']) for e in test_data.get('edadesPosibles', {}).get('edades', [])]
                )
                
                objetivo = st.selectbox(
                    "Objetivo de inversión",
                    options=[(o['id'], o['nombre']) for o in test_data.get('objetivosInversion', {}).get('objetivos', [])]
                )
            
            with col2:
                poliza = st.selectbox(
                    "Póliza de seguro",
                    options=[(p['id'], p['nombre']) for p in test_data.get('polizasSeguro', {}).get('polizas', [])]
                )
                
                capacidad = st.selectbox(
                    "Capacidad de ahorro",
                    options=[(c['id'], c['nombre']) for c in test_data.get('capacidadesAhorro', {}).get('capacidadesAhorro', [])]
                )
                
                patrimonio = st.selectbox(
                    "Porcentaje del patrimonio a invertir",
                    options=[(p['id'], p['nombre']) for p in test_data.get('porcentajesPatrimonioDedicado', {}).get('porcentajesPatrimonioDedicado', [])]
                )
            
            enviar_email = st.checkbox("Enviar resultados por email al cliente", value=True)
            
            if st.form_submit_button("📤 Enviar Test"):
                respuestas = {
                    "enviarEmailCliente": enviar_email,
                    "instrumentosInvertidosAnteriormente": [i[0] for i in instrumentos_seleccionados],
                    "nivelesConocimientoInstrumentos": niveles_conocimiento,
                    "idPlazoElegido": plazo[0] if plazo else 0,
                    "idEdadElegida": edad[0] if edad else 0,
                    "idObjetivoInversionElegida": objetivo[0] if objetivo else 0,
                    "idPolizaElegida": poliza[0] if poliza else 0,
                    "idCapacidadAhorroElegida": capacidad[0] if capacidad else 0,
                    "idPorcentajePatrimonioDedicado": patrimonio[0] if patrimonio else 0
                }
                
                resultado = enviar_test_inversor(token_portador, id_cliente, respuestas)
                if resultado:
                    st.success("✅ Test enviado exitosamente")
                    
                    # Mostrar perfil sugerido
                    perfil = resultado.get('perfilSugerido', {})
                    st.markdown(f"### 📊 Perfil Sugerido: {perfil.get('nombre', 'No disponible')}")
                    st.markdown(perfil.get('detalle', ''))
                    
                    # Mostrar composición sugerida
                    composiciones = perfil.get('perfilComposiciones', [])
                    if composiciones:
                        df_comp = pd.DataFrame(composiciones)
                        fig = go.Figure(data=[
                            go.Pie(
                                labels=df_comp['nombre'],
                                values=df_comp['porcentaje'],
                                textinfo='label+percent'
                            )
                        ])
                        st.plotly_chart(fig)
                    
                    # Comparar con composición actual
                    st.markdown("### 📈 Comparación con Portafolio Actual")
                    # Esta función debe implementarse según la estructura actual del portafolio
                    comparar_con_portafolio_actual(composiciones)

def comparar_con_portafolio_actual(composicion_sugerida):
    """
    Compara la composición sugerida con el portafolio actual
    """
