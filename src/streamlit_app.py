...
class manager:
    def __init__(self, rics, notional, directory=None):
        self.rics = rics
        self.notional = notional
        self.directory = directory or os.path.join(os.path.dirname(__file__), 'data')
    ...
    def compute_covariance(self):
        if not self.data:
            raise ValueError("No hay datos cargados para calcular covarianza")
            
        # Crear DataFrame de retornos
        returns_df = pd.DataFrame({ric: self.data[ric]['return'] for ric in self.rics})
    ...
    def compute_portfolio(self, strategy, target_return=None):
        ...
        if strategy == 'min-variance-l1':
            weights = self._min_variance_l1()
        elif strategy == 'min-variance-l2':
            weights = self._min_variance_l2()
        elif strategy == 'long-only':
            weights = self._long_only()
        elif strategy == 'equi-weight':
            weights = np.ones(len(self.rics)) / len(self.rics)
        elif strategy == 'markowitz':
            weights = self._markowitz(target_return)
        else:
            raise ValueError(f"Estrategia no soportada: {strategy}")
            
        portfolio_returns = (self.returns * weights).sum(axis=1)
        return output(portfolio_returns)
        
    def _min_variance_l1(self):
        # Implementación de mínima varianza con regularización L1
        n_assets = len(self.rics)
        initial_weights = np.ones(n_assets) / n_assets
        
        def objective(w):
            return w.T @ self.cov_matrix @ w + 0.01 * np.sum(np.abs(w))
            
        constraints = [{'type': 'eq', 'fun': lambda w: np.sum(w) - 1}]
        bounds = [(-1, 1) for _ in range(n_assets)]
        
        result = op.minimize(objective, initial_weights, 
                           method='SLSQP', bounds=bounds, constraints=constraints)
        return result.x
        
    def _min_variance_l2(self):
        # Implementación clásica de mínima varianza
        n_assets = len(self.rics)
        inv_cov = np.linalg.inv(self.cov_matrix)
        ones = np.ones(n_assets)
        return inv_cov.dot(ones) / ones.dot(inv_cov).dot(ones)
        
    def _long_only(self):
        # Portafolio con solo posiciones largas
        n_assets = len(self.rics)
        initial_weights = np.ones(n_assets) / n_assets
    ...
    def _markowitz(self, target_return=None):
        ...
        result = op.minimize(objective, np.ones(n_assets)/n_assets, 
                           method='SLSQP', bounds=bounds, constraints=constraints)
        return result.x
    ...
    if cuentas_por_moneda:
        ...
        if monedas_data:
            ...
            if columnas_disponibles:
                st.dataframe(df_monedas[columnas_disponibles].head(10))
            else:
                st.dataframe(df_monedas.head(10))
