...
def obtener_serie_historica(simbolo, mercado, fecha_desde, fecha_hasta, ajustada, bearer_token):
    """
    Obtiene la serie histórica para bonos, acciones, FCIs, opciones y cedears.
    Args:
        simbolo: Símbolo del activo (ej: 'AL30', 'GGAL', 'YPFD')
        mercado: Mercado donde cotiza (ej: 'BCBA', 'NYSE')
        fecha_desde: Fecha inicio (formato 'YYYY-MM-DD')
        fecha_hasta: Fecha fin (formato 'YYYY-MM-DD')
        ajustada: 'Ajustada' o 'SinAjustar' (solo para bonos)
        bearer_token: Token de autenticación
    """
    # Construir URL según tipo de activo
    if simbolo.endswith('D') or (len(simbolo) >= 3 and simbolo[-1].isdigit()):
        # Para bonos (incluyendo clase D)
        url = f"https://api.invertironline.com/api/v2/{mercado}/Titulos/{simbolo}/Cotizacion/seriehistorica/{fecha_desde}/{fecha_hasta}/{ajustada}"
    else:
        # Para acciones, FCIs, opciones, cedears
        url = f"https://api.invertironline.com/api/v2/{mercado}/Titulos/{simbolo}/Cotizacion/seriehistorica/{fecha_desde}/{fecha_hasta}"
    ...
    try:
        ...
        # Procesamiento común para todas las series
        ...
        print(f"Error obteniendo {simbolo}: {str(e)}")
        return None

def obtener_series_historicas_aleatorias_con_capital(
    tickers_por_panel, paneles_seleccionados, cantidad_activos, fecha_desde,
    fecha_hasta, bearer_token, capital_ars
):
    """
    Obtiene series históricas de activos aleatorios dentro del capital disponible.
    Args:
        tickers_por_panel: Dict con tickers por panel
        paneles_seleccionados: Lista de paneles a procesar
        cantidad_activos: Máximo de activos por panel
        fecha_desde: Fecha inicio periodo
        fecha_hasta: Fecha fin periodo
        bearer_token: Token de autenticación
    ...
    for panel in paneles_seleccionados:
        if panel in tickers_por_panel:
            ...
            for simbolo in tickers:
                mercado = 'BCBA'
                serie = obtener_serie_historica(
                    simbolo, mercado, fecha_desde, fecha_hasta, 'SinAjustar', bearer_token
                )
                
                if serie is not None and not serie.empty:
                    # Identificar columna de precio
                    col_precio = next(
                        (c for c in ['ultimoPrecio', 'ultimo_precio', 'precio', 'close', 'cierre'] 
                         if c in serie.columns),
                        None
                    )
                    
                    if col_precio:
                        precio = serie[col_precio].iloc[-1]
                        if precio <= capital_restante:
                            seleccionados.append((simbolo, serie, precio))
                            capital_restante -= precio
                            
                            if len(seleccionados) >= cantidad_activos:
                                break
            
            # Consolidar resultados del panel
            if seleccionados:
                for simbolo, df, _ in seleccionados:
                    df['panel'] = panel
