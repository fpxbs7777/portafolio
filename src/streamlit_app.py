...
def aplicar_estrategia_twap(cantidad_total, horario_inicio='08:00', horario_fin='16:30', zona_horaria='Europe/London'):
    """
    Aplica la estrategia Time-Weighted Average Price (TWAP) para distribuir una orden en el tiempo.
    
    Args:
        cantidad_total (int): Número total de acciones a negociar
        horario_inicio (str): Hora de inicio del mercado (HH:MM)
        horario_fin (str): Hora de fin del mercado (HH:MM)
        zona_horaria (str): Zona horaria del mercado
        
    Returns:
        dict: Diccionario con el cronograma de ejecución
    """
    from datetime import datetime
    import pytz
    
    # Convertir horarios a objetos datetime
    tz = pytz.timezone(zona_horaria)
    inicio = datetime.strptime(horario_inicio, '%H:%M').replace(tzinfo=tz)
    fin = datetime.strptime(horario_fin, '%H:%M').replace(tzinfo=tz)
    
    # Calcular duración total en minutos
    duracion_minutos = int((fin - inicio).total_seconds() / 60)
    
    # Distribuir uniformemente en intervalos de 10 minutos
    intervalos = duracion_minutos // 10
    cantidad_por_intervalo = cantidad_total / intervalos if intervalos > 0 else cantidad_total
    
    cronograma = {}
    tiempo_actual = inicio
    for i in range(intervalos):
        cronograma[tiempo_actual.strftime('%H:%M')] = round(cantidad_por_intervalo)
        tiempo_actual += timedelta(minutes=10)
    
    return cronograma

def aplicar_estrategia_vwap(cantidad_total, curva_volumen=None):
    """
    Aplica la estrategia Volume-Weighted Average Price (VWAP) para distribuir una orden según volúmenes históricos.
    
    Args:
        cantidad_total (int): Número total de acciones a negociar
        curva_volumen (dict): Diccionario con intervalos de tiempo y porcentaje de volumen histórico
                             Ejemplo: {'09:00-09:30': 0.047, '16:00-16:30': 0.112}
    Returns:
        dict: Diccionario con el cronograma de ejecución
    """
    if curva_volumen is None:
        # Curva de volumen por defecto si no se proporciona una específica
        curva_volumen = {
            '09:00-09:30': 0.047,
            '09:30-10:00': 0.053,
            '10:00-10:30': 0.055,
    ...
    cronograma = {}
    for intervalo, porcentaje in curva_volumen.items():
        cantidad = cantidad_total * porcentaje
        cronograma[intervalo] = round(cantidad)
    
    return cronograma

# --- TradingView Chart Integration ---

def crear_grafico_tradingview(symbol, interval="1D", theme="dark", width="100%", height="500"):
    """
    Crea un gráfico interactivo de TradingView para un símbolo dado.
    
    Args:
        symbol (str): Símbolo del activo (ej: 'AAPL', 'BCBA:GGAL')
        interval (str): Intervalo de tiempo ('1', '5', '15', '60', '1D', '1W', '1M')
        theme (str): Tema del gráfico ('light' o 'dark')
        width (str): Ancho del gráfico (ej: '100%', '800px')
        height (str): Alto del gráfico (ej: '500px', '100%')
        
    Returns:
        components.html: Componente HTML para mostrar en Streamlit
    """
    # Widget HTML para TradingView
    widget_html = f"""
    ...
    """
    return components.html(widget_html, height=500)

# Actualización de la función mostrar_analisis_tecnico para usar TradingView

def mostrar_analisis_tecnico(token_acceso, id_cliente):
    """Muestra análisis técnico con gráficos interactivos de TradingView"""
    ...
    # Crear pestañas para cada activo
    tabs = st.tabs([f"{activo['simbolo']} ({activo['mercado']})" for activo in portafolio['activos']])
    
    for idx, (tab, activo) in enumerate(zip(tabs, portafolio['activos'])):
        with tab:
            col1, col2 = st.columns([3, 1])
            
            with col1:
                # Mostrar gráfico de TradingView
                symbol = f"{activo['mercado'].upper()}:{activo['simbolo']}"
                crear_grafico_tradingview(
                    symbol=symbol,
                    ...
                    theme="dark"  # Coincide con el tema oscuro de tu aplicación
                )
            
            with col2:
                st.metric("Símbolo", activo['simbolo'])
                st.metric("Mercado", activo['mercado'])
                st.metric("Cantidad", f"{activo['cantidad']:,}")
                st.metric("Último Precio", f"${activo['ultimo_precio']:,.2f}")
                st.metric("Variación", f"{activo['variacion']:,.2f}%")
                
                # Botón para ver más detalles
                if st.button("Ver detalles", key=f"detalles_{idx}"):
                    st.session_state[f"show_details_{idx}"] = not st.session_state.get(f"show_details_{idx}", False)
                
                if st.session_state.get(f"show_details_{idx}", False):
                    st.json(activo)
    
    # Sección de análisis técnico personalizado
    st.markdown("---")
    st.subheader("Análisis Técnico Personalizado")
    
    # Selector de indicadores técnicos
    indicadores = st.multiselect(
        "Seleccione indicadores técnicos",
        ["RSI", "MACD", "Medias Móviles", "Bollinger Bands", "Volumen"],
        default=["RSI", "MACD"]
    )
    
    # Configuración de indicadores seleccionados
    if indicadores:
        st.info("Los indicadores seleccionados se mostrarán en el gráfico superior.")
    
    # Sección de Estrategias Algorítmicas
    st.markdown("---")
    st.subheader("Estrategias Algorítmicas de Trading")
    
    estrategia = st.radio(
        "Seleccione Estrategia de Ejecución",
        ["TWAP (Time-Weighted Average Price)", "VWAP (Volume-Weighted Average Price)"]
    )
    
    cantidad_total = st.number_input("Cantidad Total de Acciones", min_value=1, value=1000, step=100)
    
    if estrategia == "TWAP (Time-Weighted Average Price)":
        horario_inicio = st.text_input("Hora de Inicio (HH:MM)", value="08:00")
        horario_fin = st.text_input("Hora de Fin (HH:MM)", value="16:30")
        zona_horaria = st.text_input("Zona Horaria", value="Europe/London")
        
        if st.button("Aplicar Estrategia TWAP"):
            cronograma = aplicar_estrategia_twap(cantidad_total, horario_inicio, horario_fin, zona_horaria)
            st.write("Cronograma de Ejecución TWAP:")
            st.table(cronograma)
    else:  # VWAP
        if st.button("Aplicar Estrategia VWAP"):
            cronograma = aplicar_estrategia_vwap(cantidad_total)
            st.write("Cronograma de Ejecución VWAP:")
            st.table(cronograma)
