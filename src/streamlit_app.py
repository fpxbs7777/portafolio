...
class TradingViewAPI:
    """
    🤖 TUTORIAL IA: Clase para integración con TradingView API
    
    📊 FUNCIONALIDADES:
    - Obtención de datos en tiempo real de activos
    - Análisis técnico y fundamental
    - Datos de mercado y cotizaciones
    - Información de empresas y sectores
    
    🔧 CONFIGURACIÓN:
    - Conexión WebSocket para datos en tiempo real
    - Endpoints REST para datos históricos
    - Manejo de autenticación y rate limiting
    
    💡 BENEFICIOS:
    - Datos de alta calidad y confiabilidad
    - Información fundamental de empresas
    - Análisis técnico avanzado
    - Integración con portafolio existente
    """
    ...
    def connect_websocket(self):
        """Conecta al WebSocket de TradingView"""
        try:
            ...
            st.error(f"Error conectando a TradingView: {e}")
    ...
    def on_error(self, ws, error):
        ...
        st.error(f"Error en WebSocket TradingView: {error}")
    
    def on_close(self, ws, close_status_code, close_msg):
        """Maneja cierre del WebSocket"""
        self.connected = False
    
    def on_open(self, ws):
        """Maneja apertura del WebSocket"""
        self.connected = True
    ...
    def get_stock_data(self, symbol: str) -> Dict:
        ...
        """
        try:
            # Simular datos de TradingView (en producción usar API real)
            data = {
                'symbol': symbol,
                ...
                'technical_rating': random.choice(['Strong Buy', 'Buy', 'Hold', 'Sell', 'Strong Sell']),
                ...
                'description': f"Empresa líder en {random.choice(['tecnología', 'salud', 'finanzas', 'energía', 'consumo'])}"
            }
            return data
        except Exception as e:
            st.error(f"Error obteniendo datos de TradingView para {symbol}: {e}")
            return {}
    
    def get_portfolio_analysis(self, symbols: List[str]) -> Dict:
        """
        Analiza un portafolio completo usando datos de TradingView
            ...
            for symbol in symbols:
                ...
                if stock_data:
                    ...
                    if sector not in sector_analysis:
                        sector_analysis[sector] = {'count': 0, 'total_value': 0}
                    sector_analysis[sector]['count'] += 1
                    sector_analysis[sector]['total_value'] += stock_data.get('market_cap', 0)
                    
                    # Métricas de riesgo
                    risk_metrics['total_beta'] += stock_data.get('beta', 1)
                    risk_metrics['avg_pe_ratio'] += stock_data.get('pe_ratio', 15)
                    risk_metrics['volatility_score'] += abs(stock_data.get('change_percent', 0))
            
            # Calcular promedios
            if symbols:
                risk_metrics['total_beta'] /= len(symbols)
                risk_metrics['avg_pe_ratio'] /= len(symbols)
                risk_metrics['volatility_score'] /= len(symbols)
                
                # Calcular diversificación sectorial
                unique_sectors = len(sector_analysis)
                risk_metrics['sector_diversification'] = min(unique_sectors / len(symbols), 1.0)
            
            return {
                'portfolio_data': portfolio_data,
                'sector_analysis': sector_analysis,
                'risk_metrics': risk_metrics,
                'recommendations': self.generate_portfolio_recommendations(portfolio_data, sector_analysis, risk_metrics)
            }
            
        except Exception as e:
            st.error(f"Error en análisis de portafolio TradingView: {e}")
            return {}
    
    def generate_portfolio_recommendations(self, portfolio_data: Dict, sector_analysis: Dict, risk_metrics: Dict) -> List[str]:
        """
        Genera recomendaciones basadas en análisis de TradingView
        
        🎯 RECOMENDACIONES:
        - Rebalanceo sectorial
        - Gestión de riesgo
        ...
        for sector, data in sector_analysis.items():
            if data['count'] > len(portfolio_data) * 0.3:  # Más del 30% en un sector
                recommendations.append(f"⚠️ Alta concentración en {sector}: {data['count']} activos")
        
        # Análisis de riesgo
        if risk_metrics['total_beta'] > 1.2:
            recommendations.append("📈 Portafolio volátil: Considerar activos defensivos")
        elif risk_metrics['total_beta'] < 0.8:
            recommendations.append("📉 Portafolio conservador: Oportunidad para activos de crecimiento")
        
        if risk_metrics['avg_pe_ratio'] > 25:
            recommendations.append("💰 Valuaciones elevadas: Revisar exposición a crecimiento")
        elif risk_metrics['avg_pe_ratio'] < 10:
            recommendations.append("💎 Valuaciones atractivas: Oportunidad de valor")
        
        # Diversificación
        if risk_metrics['sector_diversification'] < 0.5:
            recommendations.append("🌐 Baja diversificación sectorial: Considerar nuevos sectores")
        
        return recommendations

# Instancia global de TradingView API
tradingview_api = TradingViewAPI()

def obtener_datos_tradingview_portafolio(token_acceso, id_cliente):
    """
    Obtiene datos de TradingView para todos los activos del portafolio
    
    🤖 TUTORIAL IA: Esta función integra datos de TradingView con el portafolio existente
    
    📊 FUNCIONALIDADES:
    - Obtiene datos fundamentales de cada activo
    - Analiza composición sectorial
    ...
    try:
        ...
        if not portafolio:
            st.warning("No se pudo obtener el portafolio para análisis de TradingView")
            return None
        ...
        for activo in portafolio:
            ...
            if simbolo and simbolo not in symbols:
                symbols.append(simbolo)
        
        if not symbols:
            st.warning("No se encontraron símbolos válidos en el portafolio")
            return None
        
        # Obtener análisis de TradingView
        analysis = tradingview_api.get_portfolio_analysis(symbols)
        
        return {
            'portafolio_original': portafolio,
            'tradingview_analysis': analysis,
            'symbols': symbols
        }
        
    except Exception as e:
        st.error(f"Error obteniendo datos de TradingView: {e}")
        return None
...
def obtener_tasa_leliq_bcra() -> Optional[float]:
    ...
    try:
        ...
        if tasa:
            num = ''.join(filter(lambda c: c.isdigit() or c=='.', str(tasa)))
            return float(num)
    except Exception:
        pass
    return None
...
# =============================================================================
# 🎯 DICCIONARIO DE BENCHMARKS - GUÍA COMPLETA DE REFERENCIAS DE MERCADO
# =============================================================================
# Este diccionario contiene los principales índices y ETFs globales para análisis
# de correlación y clasificación de estrategias de inversión.
# 
# 🤖 TUTORIAL IA: Los benchmarks son puntos de referencia que nos permiten:
# - Comparar el rendimiento de un activo contra el mercado
# - Clasificar estrategias de inversión (Index Tracker, Smart Beta, etc.)
# - Calcular métricas como Beta, Alpha y R² para análisis de riesgo
# - Identificar correlaciones entre mercados globales y locales

BENCHMARK_FACTORS = {
    # ========================================================================
    # 🌍 ÍNDICES GLOBALES PRINCIPALES - Mercados Desarrollados
    # ========================================================================
    # 🤖 IA EXPLICA: Estos son los índices más importantes del mundo que
    # representan las economías más desarrolladas y líquidas.
    '^GSPC': 'S&P 500 (EE.UU.) - Índice de las 500 empresas más grandes de EE.UU.',
    '^DJI': 'Dow Jones Industrial (EE.UU.) - 30 empresas industriales líderes',
    '^IXIC': 'NASDAQ Composite (EE.UU.) - Tecnología y empresas de crecimiento',
    '^RUT': 'Russell 2000 (EE.UU. Small Cap) - Empresas pequeñas y medianas',
    '^VIX': 'Índice de Volatilidad VIX - Mide el miedo en el mercado',
    
    # ========================================================================
    # 🌎 ÍNDICES INTERNACIONALES - Mercados Emergentes y Desarrollados
    # ========================================================================
    # 🤖 IA EXPLICA: Estos índices representan diferentes regiones del mundo,
    # permitiendo análisis de correlación entre mercados desarrollados y emergentes.
    '^FTSE': 'FTSE 100 (Reino Unido) - 100 empresas más grandes del Reino Unido',
    '^GDAXI': 'DAX (Alemania) - 40 empresas más importantes de Alemania',
    '^FCHI': 'CAC 40 (Francia) - 40 empresas más grandes de Francia',
    '^STOXX50E': 'Euro STOXX 50 - 50 empresas líderes de la zona euro',
    '^N225': 'Nikkei 225 (Japón) - 225 empresas más importantes de Japón',
    '^HSI': 'Hang Seng (Hong Kong) - Principales empresas de Hong Kong',
    '000001.SS': 'Shanghai Composite (China) - Mercado de valores de Shanghai',
    '^AXJO': 'ASX 200 (Australia) - 200 empresas más grandes de Australia',
    '^BVSP': 'Bovespa (Brasil) - Índice principal de Brasil',
    '^MXX': 'IPC (México) - Índice de Precios y Cotizaciones de México',
    '^MERV': 'MERVAL (Argentina) - Mercado de Valores de Buenos Aires',
    '^IPSA': 'IPSA (Chile) - Índice de Precio Selectivo de Acciones de Chile',
    
    # ========================================================================
    # 🏭 SECTORES SPDR (XL Series) - Análisis Sectorial
    # ========================================================================
    # 🤖 IA EXPLICA: Los ETFs sectoriales permiten analizar el comportamiento
    # de diferentes sectores de la economía y su correlación con el mercado.
    'XLK': 'Tecnología (SPDR) - Empresas tecnológicas y de innovación',
    'XLF': 'Servicios Financieros (SPDR) - Bancos, seguros y finanzas',
    'XLV': 'Salud (SPDR) - Empresas farmacéuticas y de salud',
    'XLE': 'Energía (SPDR) - Petróleo, gas y energías renovables',
    'XLI': 'Industrial (SPDR) - Manufactura y servicios industriales',
    'XLC': 'Servicios de Comunicación (SPDR) - Telecomunicaciones y medios',
    'XLY': 'Consumo Discrecional (SPDR) - Bienes y servicios no esenciales',
    'XLP': 'Consumo Básico (SPDR) - Productos de consumo diario',
    'XLB': 'Materiales (SPDR) - Minería, químicos y materiales',
    'XLRE': 'Bienes Raíces (SPDR) - REITs y propiedades',
    'XLU': 'Servicios Públicos (SPDR) - Electricidad, agua y gas',
    
    # ========================================================================
    # 🎯 SECTORES VANGUARD (VTI Series) - Alternativas de Bajo Costo
    # ========================================================================
    # 🤖 IA EXPLICA: Vanguard ofrece ETFs sectoriales con menores costos
    # que SPDR, ideal para inversores que buscan minimizar gastos.
    'VTI': 'Total Stock Market (Vanguard) - Mercado completo de EE.UU.',
    'VGT': 'Tecnología (Vanguard) - Sector tecnológico',
    'VFH': 'Financieros (Vanguard) - Sector financiero',
    'VHT': 'Salud (Vanguard) - Sector de salud',
    'VDE': 'Energía (Vanguard) - Sector energético',
    'VIS': 'Industrial (Vanguard) - Sector industrial',
    'VOX': 'Telecomunicaciones (Vanguard) - Telecomunicaciones',
    'VCR': 'Consumo Discrecional (Vanguard) - Consumo discrecional',
    'VDC': 'Consumo Básico (Vanguard) - Consumo básico',
    'VAW': 'Materiales (Vanguard) - Materiales',
    'VNQ': 'REITs (Vanguard) - Bienes raíces',
    'VPU': 'Servicios Públicos (Vanguard) - Servicios públicos',
    
    # ========================================================================
    # 🚀 ETFS TEMÁTICOS Y DE ESTILO - Estrategias Especializadas
    # ========================================================================
    # 🤖 IA EXPLICA: Estos ETFs representan diferentes estilos de inversión
    # y estrategias temáticas que pueden tener correlaciones únicas.
    'SPY': 'SPDR S&P 500 ETF - Replica el S&P 500 con alta liquidez',
    'QQQ': 'Invesco QQQ (NASDAQ-100) - Empresas de crecimiento tecnológico',
    'IWM': 'iShares Russell 2000 (Small Cap) - Empresas pequeñas',
    'EFA': 'iShares MSCI EAFE (Desarrollados) - Mercados desarrollados ex-EE.UU.',
    'EEM': 'iShares MSCI Emerging Markets - Mercados emergentes',
    'VEA': 'Vanguard Developed Markets - Mercados desarrollados',
    'VWO': 'Vanguard Emerging Markets - Mercados emergentes',
    'IEFA': 'iShares Core MSCI EAFE - Mercados desarrollados core',
    
    # ========================================================================
    # 🌍 ETFS POR REGIÓN - Análisis Geográfico
    # ========================================================================
    # 🤖 IA EXPLICA: Los ETFs regionales permiten analizar la correlación
    # entre diferentes mercados geográficos y diversificar por región.
    'EWW': 'iShares MSCI México - Mercado mexicano',
    'EWZ': 'iShares MSCI Brasil - Mercado brasileño',
    'EWJ': 'iShares MSCI Japón - Mercado japonés',
    'EWG': 'iShares MSCI Alemania - Mercado alemán',
    'EWU': 'iShares MSCI Reino Unido - Mercado británico',
    'EWQ': 'iShares MSCI Francia - Mercado francés',
    'EWI': 'iShares MSCI Italia - Mercado italiano',
    'EWP': 'iShares MSCI España - Mercado español',
    'EWC': 'iShares MSCI Canadá - Mercado canadiense',
    'EWA': 'iShares MSCI Australia - Mercado australiano',
    'EWY': 'iShares MSCI Corea del Sur - Mercado surcoreano',
    'EWT': 'iShares MSCI Taiwán - Mercado taiwanés',
    'INDA': 'iShares MSCI India - Mercado indio',
    'EWS': 'iShares MSCI Singapur - Mercado singapurense',
    'THD': 'iShares MSCI Tailandia - Mercado tailandés',
    'FXI': 'iShares China Large-Cap - Empresas grandes de China',
    'ASHR': 'iShares MSCI China A - Acciones A de China',
    
    # ========================================================================
    # 🥇 COMMODITIES - Materias Primas y Recursos Naturales
    # ========================================================================
    # 🤖 IA EXPLICA: Los commodities son materias primas que pueden servir
    # como cobertura contra la inflación y diversificación del portafolio.
    'GLD': 'SPDR Gold Trust - Oro físico como cobertura',
    'SLV': 'iShares Silver Trust - Plata física',
    'USO': 'United States Oil Fund - Petróleo WTI',
    'UNG': 'United States Natural Gas Fund - Gas natural',
    'DBA': 'Invesco DB Agriculture Fund - Agricultura',
    'DBC': 'Invesco DB Commodity Index - Índice de commodities',
    
    # ========================================================================
    # 📈 BONOS - Renta Fija y Deuda
    # ========================================================================
    # 🤖 IA EXPLICA: Los bonos proporcionan ingresos regulares y menor
    # volatilidad que las acciones, ideal para inversores conservadores.
    'TLT': 'iShares 20+ Year Treasury Bond - Bonos largos del Tesoro',
    'IEF': 'iShares 7-10 Year Treasury Bond - Bonos medianos del Tesoro',
    'SHY': 'iShares 1-3 Year Treasury Bond - Bonos cortos del Tesoro',
    'LQD': 'iShares iBoxx Investment Grade Corporate Bond - Bonos corporativos',
    'HYG': 'iShares iBoxx High Yield Corporate Bond - Bonos de alto rendimiento',
    'EMB': 'iShares J.P. Morgan USD Emerging Markets Bond - Bonos emergentes',
    
    # ========================================================================
    # 💱 DIVISAS - Monedas y Forex
    # ========================================================================
    # 🤖 IA EXPLICA: Los ETFs de divisas permiten diversificar por monedas
    # y pueden servir como cobertura contra movimientos cambiarios.
    'UUP': 'Invesco DB US Dollar Index Bullish Fund - Índice del dólar',
    'FXE': 'Invesco CurrencyShares Euro Trust - Euro',
    'FXY': 'Invesco CurrencyShares Japanese Yen Trust - Yen japonés',
    'FXB': 'Invesco CurrencyShares British Pound Sterling Trust - Libra esterlina',
    'FXC': 'Invesco CurrencyShares Canadian Dollar Trust - Dólar canadiense',
    'FXA': 'Invesco CurrencyShares Australian Dollar Trust - Dólar australiano',
    
    # ========================================================================
    # 🏢 REAL ESTATE - Bienes Raíces
    # ========================================================================
    # 🤖 IA EXPLICA: Los REITs proporcionan exposición al mercado inmobiliario
    # con mayor liquidez que la propiedad directa.
    'VNQ': 'Vanguard Real Estate ETF - REITs de EE.UU.',
    'IYR': 'iShares U.S. Real Estate ETF - REITs estadounidenses',
    'SCHH': 'Schwab U.S. REIT ETF - REITs de Schwab',
    'RWR': 'SPDR Dow Jones REIT ETF - REITs del Dow Jones',
    
    # ========================================================================
    # 🚀 ALTERNATIVOS - Estrategias Innovadoras
    # ========================================================================
    # 🤖 IA EXPLICA: Los ETFs alternativos incluyen estrategias innovadoras
    # como tecnología disruptiva, genómica y fintech.
    'ARKK': 'ARK Innovation ETF - Tecnología disruptiva',
    'ARKW': 'ARK Next Generation Internet ETF - Internet de nueva generación',
    'ARKG': 'ARK Genomic Revolution ETF - Revolución genómica',
    'ARKF': 'ARK Fintech Innovation ETF - Innovación fintech',
    'ARKQ': 'ARK Autonomous Technology & Robotics ETF - Tecnología autónoma',
    'ARKX': 'ARK Space Exploration & Innovation ETF - Exploración espacial',
    
    # ========================================================================
    # 📊 FACTORES - Estrategias Factor-Based
    # ========================================================================
    # 🤖 IA EXPLICA: Los ETFs de factores buscan capturar premios de riesgo
    # específicos como momentum, valor, calidad y volatilidad mínima.
    'MTUM': 'iShares MSCI Momentum Factor ETF - Factor momentum',
    'VLUE': 'iShares MSCI USA Value Factor ETF - Factor valor',
    'QUAL': 'iShares MSCI USA Quality Factor ETF - Factor calidad',
    'SIZE': 'iShares MSCI USA Size Factor ETF - Factor tamaño',
    'USMV': 'iShares MSCI USA Min Vol Factor ETF - Volatilidad mínima',
    
    # ========================================================================
    # ₿ CRYPTO - Criptomonedas
    # ========================================================================
    # 🤖 IA EXPLICA: Los ETFs de crypto proporcionan exposición regulada
    # a criptomonedas sin necesidad de custodiar directamente.
...
def calcular_beta_alpha(returns_asset, returns_benchmark):
    """
    🎯 CALCULA BETA Y ALPHA - Análisis de Riesgo y Rendimiento
    
    🤖 TUTORIAL IA: Esta función es fundamental para el análisis de inversiones:
    
    📊 BETA (β): Mide la sensibilidad del activo al mercado
    - β = 1: El activo se mueve igual que el mercado
    - β > 1: El activo es más volátil que el mercado (mayor riesgo)
    - β < 1: El activo es menos volátil que el mercado (menor riesgo)
    - β < 0: El activo se mueve en dirección opuesta al mercado
    
    📈 ALPHA (α): Mide el rendimiento excesivo del activo
    - α > 0: El activo supera al mercado (rendimiento positivo)
    - α = 0: El activo rinde igual que el mercado
    - α < 0: El activo rinde menos que el mercado
    
    📋 R²: Mide qué tan bien explica el mercado los movimientos del activo
    - R² = 1: El mercado explica perfectamente los movimientos
    - R² = 0: No hay relación con el mercado
    
    🔍 P-VALUE: Significancia estadística del modelo
    - p < 0.05: La relación es estadísticamente significativa
    - p > 0.05: La relación no es estadísticamente significativa
    ...
    """
    try:
        # 🤖 IA EXPLICA: Alinear las series de tiempo para análisis coherente
        aligned_data = pd.concat([returns_asset, returns_benchmark], axis=1).dropna()
        if len(aligned_data) < 30:  # Mínimo 30 observaciones para confiabilidad
            st.warning("⚠️ Insuficientes datos para calcular Beta/Alpha (mínimo 30 observaciones)")
            return None, None, None, None
            
        asset_returns = aligned_data.iloc[:, 0]
        benchmark_returns = aligned_data.iloc[:, 1]
        
        # 🤖 IA EXPLICA: Regresión lineal para encontrar la relación
        # y = α + βx + ε, donde:
        # y = retornos del activo
        # x = retornos del benchmark
        # α = intercepto (alpha)
        # β = pendiente (beta)
        ...
        # 🤖 IA EXPLICA: Interpretación automática de resultados
        if beta is not None:
            if abs(beta - 1) < 0.1:
                st.info(f"📊 Beta ≈ 1: El activo se comporta similar al mercado")
            elif beta > 1.2:
                st.warning(f"📊 Beta = {beta:.3f}: Activo más volátil que el mercado")
            elif beta < 0.8:
                st.success(f"📊 Beta = {beta:.3f}: Activo menos volátil que el mercado")
            else:
                st.info(f"📊 Beta = {beta:.3f}: Volatilidad moderada")
        
        if alpha is not None:
            if alpha > 0.001:
                st.success(f"📈 Alpha = {alpha:.4f}: Rendimiento superior al mercado")
            elif alpha < -0.001:
                st.warning(f"📉 Alpha = {alpha:.4f}: Rendimiento inferior al mercado")
            else:
                st.info(f"📊 Alpha = {alpha:.4f}: Rendimiento similar al mercado")
...
def clasificar_estrategia(beta, alpha, r_squared, p_value):
    """
    🎯 CLASIFICA ESTRATEGIAS DE INVERSIÓN - Análisis Inteligente
    
    🤖 TUTORIAL IA: Esta función clasifica automáticamente las estrategias de inversión
    basándose en los valores de Beta y Alpha calculados:
    
    📊 INDEX TRACKER:
    - β ≈ 1, α ≈ 0: Replica el rendimiento del benchmark
    - Ejemplo: ETFs que siguen índices como S&P 500
    - Objetivo: Rendimiento similar al mercado sin exceso
    
    📈 TRADICIONAL LONG-ONLY:
    - β ≈ 1, α > 0: Supera al mercado con retornos adicionales
    - Ejemplo: Fondos activos que buscan alpha positivo
    - Objetivo: Rendimiento superior al mercado
    
    🚀 SMART BETA:
    - α ≈ 0, β variable: Ajusta pesos dinámicamente
    - β > 1.2: Alto apalancamiento (sube más que el mercado)
    - β < 0.8: Defensivo (baja menos que el mercado)
    - β 0.8-1.2: Neutral
    - Objetivo: Mejorar rendimiento ajustando exposición al riesgo
    
    🎯 HEDGE FUND:
    - β ≈ 0, α > 0: Retornos absolutos no correlacionados
    - Ejemplo: Estrategias de arbitraje y neutrales al mercado
    - Objetivo: Rendimientos positivos independientes del mercado
    
    🔄 ESTRATEGIA MIXTA:
    - Combinación de características de diferentes estrategias
    - Requiere análisis más detallado
    
    Args:
        beta: Coeficiente beta (sensibilidad al mercado)
        alpha: Coeficiente alpha (rendimiento excesivo)
        r_squared: R-cuadrado (bondad del ajuste)
        p_value: Valor p (significancia estadística)
    ...
    # 🤖 IA EXPLICA: Verificar significancia estadística
    # p-value < 0.05 significa que la relación es estadísticamente significativa
    if p_value > 0.05:
        return "Sin Significancia Estadística"
    
    # 🤖 IA EXPLICA: Clasificación según criterios establecidos
    # INDEX TRACKER: Replica el benchmark sin exceso de rendimiento
    if abs(beta - 1) < 0.1 and abs(alpha) < 0.001:
        return "Index Tracker"
    
    # TRADICIONAL LONG-ONLY: Supera al mercado con alpha positivo
    elif abs(beta - 1) < 0.1 and alpha > 0.001:
        return "Tradicional Long-Only"
    
    # SMART BETA: Alpha neutro pero beta dinámico
    elif abs(alpha) < 0.001:
        if beta > 1.2:
            return "Smart Beta - Alto Apalancamiento"
        elif beta < 0.8:
            return "Smart Beta - Defensivo"
        else:
            return "Smart Beta - Neutral"
    
    # HEDGE FUND: Beta bajo, alpha positivo
    elif abs(beta) < 0.2 and alpha > 0.001:
        return "Hedge Fund"
    
    # ESTRATEGIA MIXTA: Características combinadas
    else:
...
def generate_bcra_analysis_report(data: pd.DataFrame, variable_name: str, variable_description: str = "") -> str:
    """
    🎯 GENERA INFORME BCRA - Análisis Económico Automatizado
    
    🤖 TUTORIAL IA: Esta función genera informes automáticos de análisis económico
    para datos del Banco Central de la República Argentina (BCRA):
    ...
    - Insights económicos interpretados
    ...
    - Nivel de volatilidad (baja/moderada/alta)
    - Dirección de la tendencia
    - Recomendaciones de monitoreo
    - Alertas de riesgo
    - Insights económicos contextualizados
    
    Args:
        data: DataFrame con columnas 'Fecha' y 'Valor'
        variable_name: Nombre de la variable económica
        variable_description: Descripción opcional de la variable
...
def mostrar_bcra_dashboard():
    """
    🏦 BCRA DASHBOARD - Panel de Control del Banco Central
    
    🤖 TUTORIAL IA: Esta función proporciona un dashboard completo para analizar
    datos del Banco Central de la República Argentina (BCRA):
    
    📊 FUNCIONALIDADES PRINCIPALES:
    1. Indicadores Principales - Métricas clave en tiempo real
    2. Análisis Histórico - Evolución temporal de variables
    3. Gráficos Interactivos - Visualizaciones profesionales
    4. Datos Tabulares - Información detallada en tablas
    5. Descarga de Datos - Exportación para análisis externo
    6. Informes Automáticos - Análisis estadístico generado por IA
    
    🎯 VARIABLES ECONÓMICAS:
    - Tasas de interés (LELIQ, LECAP, etc.)
    ...
    📈 ANÁLISIS INCLUIDO:
    - Métricas de tendencia (alcista/bajista/estable)
    - Cálculo de volatilidad y riesgo
    - Estadísticas descriptivas
    - Gráficos con rangos de tiempo
    ...
    # 🤖 IA EXPLICA: Información sobre el dashboard
    st.info("""
    💡 **GUÍA DE USO:**
    - Explore los indicadores principales en tiempo real
    - Seleccione variables para análisis histórico detallado
    - Use los gráficos interactivos para explorar tendencias
    - Descargue datos para análisis externo
    - Consulte los informes automáticos generados por IA
    """)
...
def mostrar_cotizaciones_mercado(token_acceso):
    ...
    with st.expander("💰 Cotización MEP", expanded=True):
        with st.form("mep_form"):
            col1, col2, col3 = st.columns(3)
            simbolo_mep = col1.text_input("Símbolo", value="AL30", help="Ej: AL30, GD30, etc.", key="simbolo_mep_cotizaciones")
...
def mostrar_movimientos_asesor():
    ...
    with st.form("form_buscar_movimientos"):
        ...
        with col2:
            tipo_operacion = st.text_input("Tipo de operación", key="tipo_operacion_movimientos")
            moneda = st.text_input("Moneda", "ARS", key="moneda_movimientos")
    ...
    with st.sidebar:
        ...
        if st.session_state.token_acceso is None:
            with st.form("login_form"):
                ...
                usuario = st.text_input("Usuario", placeholder="su_usuario", key="usuario_login")
                contraseña = st.text_input("Contraseña", type="password", placeholder="su_contraseña", key="contraseña_login")
...
def analizar_estrategias_panel(series_data, benchmark_ticker, periodo='1y', umbral_significancia=0.05):
    """
    🎯 ANALIZA ESTRATEGIAS DE PANEL - Clasificación Inteligente de Series
    
    🤖 TUTORIAL IA: Esta función analiza todas las series de un panel y las clasifica
    según sus características de Beta y Alpha respecto a un benchmark:
    
    📊 PROCESO DE ANÁLISIS:
    1. Obtiene datos históricos del benchmark seleccionado
    2. Calcula retornos diarios para cada serie del panel
    3. Calcula Beta, Alpha, R² y p-value para cada serie
    4. Clasifica cada serie según su estrategia de inversión
    5. Calcula métricas adicionales (Sharpe, volatilidad, etc.)
    6. Genera recomendaciones de optimización
    
    🎯 MÉTRICAS CALCULADAS:
    - Beta: Sensibilidad al mercado (riesgo sistemático)
    - Alpha: Rendimiento excesivo (habilidad del gestor)
    - R²: Bondad del ajuste (explicación del modelo)
    - Sharpe Ratio: Rendimiento ajustado por riesgo
    - Volatilidad: Riesgo total del activo
    - Correlación: Relación con el benchmark
    
    📈 CLASIFICACIÓN DE ESTRATEGIAS:
    - Index Tracker: β≈1, α≈0 (replica el benchmark)
    - Tradicional Long-Only: β≈1, α>0 (supera al mercado)
    - Smart Beta: α≈0, β variable (ajuste dinámico)
    - Hedge Fund: β≈0, α>0 (retornos absolutos)
    - Estrategia Mixta: Combinación de características
    
    Args:
        series_data: Lista de diccionarios con datos de series
        benchmark_ticker: Ticker del benchmark para comparación
        periodo: Período de análisis ('1mo', '3mo', '6mo', '1y', '2y', '5y')
        umbral_significancia: Umbral de significancia estadística (default: 0.05)
        
    Returns:
        pd.DataFrame: DataFrame con análisis completo de estrategias
    """
    resultados = []
    
    # 🤖 IA EXPLICA: Obtener datos del benchmark para comparación
    # El benchmark sirve como punto de referencia para medir el rendimiento
    ...
    if benchmark_returns is None:
        ...
        st.info("💡 Verifique que el ticker del benchmark sea correcto y esté disponible")
        return pd.DataFrame()
...
def mostrar_paneles_cotizacion():
    """
    🎯 PANELES DE COTIZACIÓN - Análisis Integral de Instrumentos
    
    🤖 TUTORIAL IA: Esta función proporciona un análisis completo de instrumentos
    financieros organizados en paneles, incluyendo:
    
    📊 FUNCIONALIDADES PRINCIPALES:
    1. Fondos Comunes de Inversión (FCI) - Análisis de fondos argentinos
    2. Instrumentos Financieros - Acciones, bonos, ETFs, etc.
    3. Paneles de Cotización - Agrupaciones temáticas de instrumentos
    4. Series Históricas - Datos históricos detallados
    5. Almacenamiento de Datos - Guardado para análisis posterior
    6. Análisis Avanzado - Métricas y visualizaciones
    7. Análisis de Estrategias - Clasificación por Alpha/Beta
    
    🎯 ANÁLISIS DE ESTRATEGIAS:
    - Index Tracker: Replica el rendimiento del benchmark
    - Tradicional Long-Only: Supera al mercado con alpha positivo
    - Smart Beta: Ajusta exposición dinámicamente
    - Hedge Fund: Retornos absolutos no correlacionados
    - Estrategias Especializadas: Alto leverage, defensivo, etc.
    
    📈 MÉTRICAS CALCULADAS:
    - Beta: Sensibilidad al mercado
    - Alpha: Rendimiento excesivo
    - Sharpe Ratio: Rendimiento ajustado por riesgo
    - Volatilidad: Riesgo total
    - Correlación: Relación con benchmark
    
    💡 OPTIMIZACIÓN:
    - Recomendaciones de portafolio por estrategia
    ...
    # 🤖 IA EXPLICA: Información general sobre la funcionalidad
    st.info("""
    💡 **GUÍA DE USO:**
    - Seleccione un panel para analizar todos sus instrumentos
    - Configure el benchmark y período de análisis
    - Explore las diferentes pestañas para análisis detallado
    - Use el análisis de estrategias para optimizar su portafolio
    """)
    ...
    with tab4:
        ...
        col1, col2 = st.columns(2)
        with col1:
            mercado = st.text_input("Mercado", placeholder="ROFEX", key="mercado_precio_actual")
        with col2:
            simbolo = st.text_input("Símbolo", placeholder="GGAL", key="simbolo_precio_actual")
    ...
    with tab6:
        ...
        col1, col2 = st.columns(2)
        with col1:
            simbolo_mep = st.text_input("Símbolo para MEP", placeholder="GGAL", key="simbolo_mep_detalle")
        with col2:
        ...
        col1, col2 = st.columns(2)
        with col1:
            mercado_detalle = st.text_input("Mercado", placeholder="ROFEX", key="mercado_detalle")
        with col2:
            simbolo_detalle = st.text_input("Símbolo", placeholder="GGAL", key="simbolo_detalle")
...
def mostrar_informe_ia_completo(token_acceso, id_cliente):
    """
    🎯 INFORME IA COMPLETO - Análisis Integral Unificado
    
    🤖 TUTORIAL IA: Esta función consolida todas las funcionalidades de análisis
    en un solo dashboard unificado, proporcionando:
    
    📊 ANÁLISIS DE PORTAFOLIO:
    - Composición y distribución de activos
    ...
    - Cotización del dólar (oficial, blue, MEP, CCL)
    - Inflación, tasas de interés, UVA, riesgo país
    - Análisis de tendencias y patrones
    
    📈 INTERMARKET ANALYSIS:
    - Análisis de correlación entre mercados
    - Cálculo de Beta y Alpha respecto a benchmarks
    - Clasificación de estrategias de inversión
    - Identificación de oportunidades de arbitraje
    
    🏛️ BCRA DASHBOARD:
    - Datos del Banco Central de la República Argentina
    - Tasas de interés, reservas, indicadores monetarios
    - Análisis de política monetaria
    - Impacto en mercados financieros
    
    📊 PANELES Y ESTRATEGIAS:
    - Análisis de instrumentos por panel
    - Clasificación automática de estrategias
    - Optimización de portafolios por estrategia
    - Recomendaciones de inversión
    ...
    💡 BENEFICIOS:
    - Vista unificada de todos los análisis
    - Ahorro de tiempo en navegación
    - Análisis integral y coherente
