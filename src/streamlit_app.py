...
def mostrar_optimizacion_portafolio(token_acceso, id_cliente):
    ...
    with col1:
        ...
        modo_optimizacion = st.radio(
            ...
            horizontal=True,
            key='opt_modo_optimizacion_radio'
        )
    ...
    metodo_ui = st.selectbox(
        ...
        key="opt_metodo_optimizacion_select"
    )
    ...
    scheduling_ui = st.selectbox(
        ...
        key="opt_scheduling_algo_select"
    )
...
def obtener_dolares_financieros_completo(token_portador):
    """
    Obtiene las cotizaciones de los diferentes tipos de dólar desde múltiples fuentes.
    
    Args:
        token_portador (str): Token de autenticación para la API de InvertirOnline
        
    Returns:
        dict: Diccionario con las cotizaciones de los diferentes tipos de dólar
    """
    try:
        # Diccionario para almacenar las cotizaciones
        cotizaciones = {}
        
        # Obtener cotizaciones de IOL
        headers = {
            'Authorization': f'Bearer {token_portador}',
            'Content-Type': 'application/json'
        }
        
        # 1. Obtener dólar MEP (Bonos)
        try:
            response = requests.get(
                'https://api.invertironline.com/api/v2/Cotizaciones/bonos/AL30/argentina/cedears',
                headers=headers
            )
            if response.status_code == 200:
                data = response.json()
                if data and 'ultimoPrecio' in data:
                    cotizaciones['MEP'] = {
                        'valor': round(data['ultimoPrecio'], 2),
                        'variacion': round(data.get('variacion', 0), 2) if data.get('variacion') is not None else 0.0,
                        'hora': datetime.now().strftime('%H:%M')
                    }
        except Exception as e:
            st.warning(f"No se pudo obtener el dólar MEP: {str(e)}")
        
        # 2. Obtener dólar CCL (Cedears)
        try:
            response = requests.get(
                'https://api.invertironline.com/api/v2/Cotizaciones/cedears/GGAL/nyse/cedears',
                headers=headers
            )
            if response.status_code == 200:
                data = response.json()
                if data and 'ultimoPrecio' in data:
                    # Obtener precio del subyacente en USD
                    simbolo_usa = data.get('simbolo', '').split('/')[0]
                    if simbolo_usa:
                        ticker = yf.Ticker(simbolo_usa)
                        hist = ticker.history(period='1d')
                        if not hist.empty:
                            precio_usa = hist['Close'].iloc[-1]
                            ratio = data.get('ratio', 1)
                            ccl = (data['ultimoPrecio'] / precio_usa) * ratio
                            cotizaciones['CCL'] = {
                                'valor': round(ccl, 2),
                                'variacion': round(data.get('variacion', 0), 2) if data.get('variacion') is not None else 0.0,
                                'hora': datetime.now().strftime('%H:%M')
            ...
            st.warning(f"No se pudo obtener el dólar CCL: {str(e)}")
        
        # 3. Obtener dólar Blue y otros de dolarapi.com como respaldo
        try:
            response = requests.get('https://dolarapi.com/dolares')
            if response.status_code == 200:
                data = response.json()
                for dolar in data:
                    if dolar['casa'] == 'blue':
                        cotizaciones['Blue'] = {
                            'valor': dolar['venta'],
                            'variacion': 0.0,  # La API no provee variación
                            'hora': datetime.now().strftime('%H:%M')
                        }
                    elif dolar['casa'] == 'oficial':
                        cotizaciones['Oficial'] = {
                            'valor': dolar['venta'],
                            'variacion': 0.0,
                            'hora': datetime.now().strftime('%H:%M')
                        }
                    elif dolar['casa'] == 'tarjeta':
            ...
            st.warning(f"No se pudieron obtener cotizaciones de dolarapi.com: {str(e)}")
            
        # 4. Obtener dólar mayorista de BCRA
        try:
            hoy = date.today()
            ...
            response = requests.get(
                f'https://api.estadisticasbcra.com/usd_of',
                headers={'Authorization': 'BEARER YOUR_BCRA_TOKEN'}
            )
            ...
            st.warning(f"No se pudo obtener el dólar mayorista del BCRA: {str(e)}")
        
        # 5. Obtener dólar Canje (promedio entre MEP y CCL)
        if 'MEP' in cotizaciones and 'CCL' in cotizaciones:
            valor_canje = (cotizaciones['MEP']['valor'] + cotizaciones['CCL']['valor']) / 2
            cotizaciones['Canje'] = {
                'valor': round(valor_canje, 2),
                'variacion': round((cotizaciones['MEP']['variacion'] + cotizaciones['CCL']['variacion']) / 2, 2),
                'hora': datetime.now().strftime('%H:%M')
            }
        ...
        st.error(f"Error al obtener cotizaciones de dólares: {str(e)}")
        return {}
