...
import base64
import time
# Importaciones adicionales para el dashboard del BCRA
...
def mostrar_analisis_ia_portafolio(token_acceso, id_cliente, nombre_cliente):
    """
    Muestra el análisis IA del portafolio del cliente.
    """
    st.subheader("🤖 Análisis Inteligente del Portafolio")
    st.markdown(f"### Análisis IA para {nombre_cliente}")
    
    # Verificar dependencias de IA
    deps_available = check_and_install_dependencies()
    
    if not deps_available:
        st.warning("⚠️ **Funcionando en modo limitado**: Análisis IA no disponible. El análisis estadístico básico sigue funcionando.")
    
    # Obtener portafolio actual
    portafolio = obtener_portafolio(token_acceso, id_cliente)
    if not portafolio:
        st.warning("No se pudo obtener el portafolio para análisis IA")
        return
    ...
    with col2:
        tipo_analisis = st.selectbox(
            "Tipo de análisis:",
            ["Análisis individual", "Análisis de portafolio completo", "Análisis comparativo"],
            index=0
        )
    ...
    dias_analisis = periodos[periodo_analisis]
    ...
    if st.button("🚀 Generar Análisis IA", type="primary", use_container_width=True):
        with st.spinner("🤖 Generando análisis inteligente..."):
            try:
                if tipo_analisis == "Análisis individual":
                    mostrar_analisis_individual_ia(portafolio, token_acceso, dias_analisis)
                elif tipo_analisis == "Análisis de portafolio completo":
                    mostrar_analisis_portafolio_completo_ia(portafolio, token_acceso, dias_analisis)
                else:
                    mostrar_analisis_comparativo_ia(portafolio, token_acceso, dias_analisis)
    ...
    with col1:
        ...
        try:
            ...
            st.success("✅ API IOL - Conectado")
        except Exception:
            st.error("❌ API IOL - Error de conexión")
    ...
    with col2:
        ...
        try:
            ...
            if any(keyword in error_str for keyword in ["quota", "429", "exceeded", "limit"]):
                st.warning("⚠️ Gemini AI - Cuota agotada")
            else:
                st.error("❌ Gemini AI - Error de conexión")

def mostrar_analisis_individual_ia(portafolio, token_acceso, dias_analisis):
    """
    Muestra análisis IA individual de cada activo en el portafolio.
    """
    st.subheader("📊 Análisis Individual de Activos")
    
    # Obtener activos del portafolio
    activos = []
    if 'activos' in portafolio:
        activos = portafolio['activos']
    elif isinstance(portafolio, list):
        activos = portafolio
    ...
    activo_seleccionado = st.selectbox(
        "Seleccionar activo para análisis:",
        options=[activo.get('titulo', activo.get('simbolo', 'Activo')) for activo in activos],
        index=0
    )
    ...
    activo_data = None
    for activo in activos:
        if activo.get('titulo', activo.get('simbolo', '')) == activo_seleccionado:
            activo_data = activo
            break
    
    if not activo_data:
        st.error("No se pudo obtener datos del activo seleccionado")
        return
    ...
    try:
        ...
        if 'Close' in data.columns:
            ...
            data_clean.columns = ['fecha', 'precio']
        else:
            # Datos de IOL
        ...
        with st.spinner("🤖 Generando análisis IA..."):
            report = generate_analysis_report(data_clean, simbolo, activo_seleccionado)
        
        # Mostrar el análisis
        st.markdown("### 📄 Informe de Análisis IA")
        
        # Botones de acción
        col1, col2 = st.columns([1, 1])
        
        with col1:
            if st.button("🔄 Regenerar Análisis", key="refresh_individual"):
                with st.spinner("🤖 Regenerando análisis..."):
                    new_report = generate_analysis_report(data_clean, simbolo, activo_seleccionado)
                    st.session_state.individual_report = new_report
                    st.rerun()
        
        with col2:
            if st.button("📋 Copiar Análisis", key="copy_individual"):
                st.info("💡 Selecciona y copia el texto del análisis mostrado abajo")
        ...
        if "algoritmos estadísticos" in report.lower():
            st.info("""
            📊 **Análisis generado con Algoritmos Estadísticos**  
            Este informe ha sido creado utilizando análisis estadístico automatizado. 
            Proporciona insights valiosos basados en cálculos matemáticos precisos.
            
            💡 **Para análisis con IA**: Espere unos minutos e intente regenerar el análisis.
            """)
        else:
            st.info("""
            🧠 **Análisis generado con Inteligencia Artificial**  
            Este informe ha sido creado automáticamente utilizando Google Gemini AI, 
            basado en los datos históricos del activo seleccionado.
            """)
            
    except Exception as e:
        st.error(f"Error al obtener datos históricos: {str(e)}")

def mostrar_analisis_portafolio_completo_ia(portafolio, token_acceso, dias_analisis):
    """
    Muestra análisis IA del portafolio completo.
    """
    ...
    # Generar análisis del portafolio completo
    with st.spinner("🤖 Analizando portafolio completo..."):
        try:
            # Crear resumen del portafolio
            total_activos = len(activos)
            total_valor = sum(float(activo.get('valorMercado', 0)) for activo in activos)
            
            # Agrupar por tipo de activo
            tipos_activo = {}
            for activo in activos:
                tipo = activo.get('tipo', 'Otro')
                if tipo not in tipos_activo:
                    tipos_activo[tipo] = []
                tipos_activo[tipo].append(activo)
            
            # Generar informe del portafolio
            report = f"""
# 📊 Análisis Completo del Portafolio

*Generado con IA el {datetime.now().strftime('%d/%m/%Y %H:%M')}*
...
• **Tipos de activos**: {len(tipos_activo)}

## 📈 Distribución por Tipo de Activo

"""
            
            for tipo, activos_tipo in tipos_activo.items():
                valor_tipo = sum(float(activo.get('valorMercado', 0)) for activo in activos_tipo)
                porcentaje = (valor_tipo / total_valor * 100) if total_valor > 0 else 0
                report += f"""
### {tipo.title()}
• **Cantidad**: {len(activos_tipo)} activos
• **Valor**: ${valor_tipo:,.2f}
...
### Diversificación
El portafolio muestra una diversificación en {len(tipos_activo)} tipos de activos diferentes, 
lo que contribuye a la reducción del riesgo general.

### Concentración
La distribución del capital entre los diferentes tipos de activos permite 
un balance entre riesgo y retorno potencial.

## 🎯 Recomendaciones

1. **Monitoreo continuo**: Revisar periódicamente la composición del portafolio
2. **Rebalanceo**: Considerar rebalancear cuando las proporciones se desvíen significativamente
3. **Diversificación**: Evaluar oportunidades de diversificación adicional

---

*Análisis generado con IA + datos de IOL*
"""
            
            # Mostrar el informe
            st.markdown("### 📄 Informe del Portafolio")
            ...
            st.error(f"Error en el análisis del portafolio: {str(e)}")

def mostrar_analisis_comparativo_ia(portafolio, token_acceso, dias_analisis):
    """
    Muestra análisis IA comparativo entre activos del portafolio.
    """
    ...
    # Seleccionar activos para comparar
    activos_disponibles = [activo.get('titulo', activo.get('simbolo', 'Activo')) for activo in activos]
    
    col1, col2 = st.columns(2)
    with col1:
        activo1 = st.selectbox("Primer activo:", activos_disponibles, index=0)
    with col2:
        activo2 = st.selectbox("Segundo activo:", activos_disponibles, index=min(1, len(activos_disponibles)-1))
    
    if st.button("🔄 Comparar Activos", type="primary"):
        with st.spinner("🤖 Generando análisis comparativo..."):
            try:
                # Generar análisis comparativo
                report = f"""
# 📊 Análisis Comparativo - {activo1} vs {activo2}
...
Se comparan los activos **{activo1}** y **{activo2}** del portafolio para evaluar 
sus características de riesgo y retorno.

## 📈 Análisis de Rendimiento

### {activo1}
• **Tipo**: Activo del portafolio
• **Análisis**: Evaluación de rendimiento y riesgo
...
### Diversificación
La comparación entre diferentes activos permite evaluar la diversificación 
del portafolio y identificar oportunidades de optimización.

### Gestión de Riesgo
El análisis comparativo ayuda a identificar activos con perfiles de riesgo 
complementarios para mejorar la gestión del portafolio.

## 🎯 Recomendaciones

1. **Evaluar correlación**: Analizar la correlación entre ambos activos
2. **Balance de riesgo**: Considerar el balance entre riesgo y retorno
3. **Oportunidades**: Identificar oportunidades de optimización

---

*Análisis comparativo generado con IA*
"""
                
                # Mostrar el informe
                st.markdown("### 📄 Informe Comparativo")
...
def mostrar_analisis_portafolio():
    ...
    with tab6:
        mostrar_analisis_ia_portafolio(token_acceso, id_cliente, nombre_cliente)
...
def check_and_install_dependencies():
    """Verifica e instala las dependencias necesarias para IA"""
    missing_deps = []
    ...
    if missing_deps:
        ...
        with st.expander("📦 **Instrucciones de instalación**", expanded=True):
            ...
            st.warning("⚠️ **Nota**: Sin estas dependencias, el análisis IA no funcionará, pero el análisis estadístico básico seguirá disponible.")
...
def format_data_for_prompt(data_clean: pd.DataFrame) -> str:
    """
    Formatea los datos para el prompt de manera eficiente.
    Reduce la cantidad de datos enviados para ahorrar tokens.
    """
    if len(data_clean) <= 10:
        # Si hay pocos datos, enviar todos
        return '\n'.join([f"{row['fecha'].strftime('%d/%m/%Y')}: {row['precio']:.2f}" 
...
def generate_basic_analysis_report(data: pd.DataFrame, ticker: str, ticker_description: str = "") -> str:
    """
    Genera un informe básico de análisis sin usar IA como fallback.
    """
    try:
        data_clean = data.dropna(subset=['precio', 'fecha']).copy()
        data_clean['precio'] = pd.to_numeric(data_clean['precio'], errors='coerce')
        data_clean = data_clean.dropna(subset=['precio'])
        
        if len(data_clean) == 0:
            return "## ⚠️ Sin datos válidos\n\nNo hay datos válidos para analizar."
        
        summary = data_clean['precio'].describe().to_dict()
        last_value = data_clean['precio'].iloc[-1]
        first_value = data_clean['precio'].iloc[0]
        total_change = ((last_value - first_value) / first_value * 100) if first_value != 0 else 0
        
        # Calcular métricas
        volatility = data_clean['precio'].std() / data_clean['precio'].mean() * 100 if data_clean['precio'].mean() != 0 else 0
        x_vals = np.arange(len(data_clean))
        coeffs = np.polyfit(x_vals, data_clean['precio'], 1)
...
El activo **{ticker}** mostró una tendencia **{trend}** durante el período analizado, con una variación total de **{total_change:+.2f}%**.

### Puntos Destacados
- **Precio inicial**: ${first_value:,.2f}
- **Precio final**: ${last_value:,.2f}
- **Precio máximo**: ${summary.get('max', 0):,.2f}
- **Precio mínimo**: ${summary.get('min', 0):,.2f}
...
"""
        ...
        if volatility < 5:
            report += """
- ✅ **Estabilidad alta**: El activo muestra comportamiento predecible
...
1. **Comportamiento general**: El activo mostró una tendencia **{trend}** con volatilidad **{volatility_level}**
2. **Rango de precios**: Fluctuó entre ${summary.get('min', 0):,.2f} y ${summary.get('max', 0):,.2f}
...
*Análisis generado automáticamente con algoritmos estadísticos*
"""
...
def generate_analysis_report_optimized(data: pd.DataFrame, ticker: str, ticker_description: str = "") -> str:
    """
    Genera un informe de análisis optimizado usando la API de Gemini con uso mínimo de tokens.
    """
    try:
        if data.empty or 'precio' not in data.columns or 'fecha' not in data.columns:
            return generate_basic_analysis_report(data, ticker, ticker_description)
        ...
        prompt = f"""Actúa como un analista financiero experto para el mercado argentino.
Tu tarea es analizar los datos históricos del siguiente activo y generar un informe conciso y claro en español.

**Activo a Analizar:** {ticker}
**Período de Datos:** Desde {start_date} hasta {end_date}
**Variación Total:** {total_change:+.2f}%

**Muestra de Datos (Fecha: Precio):**
{data_sample}
...
4. **Conclusión:** Ofrece una breve conclusión sobre la situación actual del activo basada en los datos proporcionados.
...
Por favor, genera el informe siguiendo esta estructura. Sé profesional y objetivo en tu análisis. Máximo 300 palabras."""
        ...
        try:
            ...
            try:
                ...
                st.warning("📊 Dependencias de IA no disponibles. Usando análisis estadístico.")
                return generate_basic_analysis_report(data_clean, ticker, ticker_description)
            
            # Configuración ultra-optimizada para ahorrar cuota
            generation_config = {
                "temperature": 0.3,  # Menor creatividad, más eficiencia
                "top_p": 0.7,
                "top_k": 10,
                "max_output_tokens": 800,  # Reducido significativamente
            }
            
            # Usar modelo más económico
            model = genai.GenerativeModel(
                model_name="gemini-1.5-flash",  # El más económico disponible
                generation_config=generation_config,
            )
            
            # Solo un intento para ahorrar cuota
            response = model.generate_content(prompt)
...
# 📊 {ticker} - Análisis de Mercado

*Generado con IA el {datetime.now().strftime('%d/%m/%Y %H:%M')} | Período: {start_date} - {end_date}*
...
*Análisis generado con Gemini AI + datos de mercado*
"""
                
                st.success("✅ Informe IA generado exitosamente")
            ...
            if any(keyword in error_str for keyword in ["quota", "429", "exceeded", "limit"]):
                st.warning("⚠️ Cuota de IA temporalmente agotada. Usando análisis estadístico avanzado.")
                
                # Generar informe básico mejorado como fallback
                basic_report = generate_basic_analysis_report(data_clean, ticker, ticker_description)
...
🔧 **Análisis generado con algoritmos estadísticos avanzados**  
Este informe utiliza técnicas de análisis cuantitativo automatizado debido a limitaciones temporales de la API de IA.

**Ventajas del análisis estadístico:**
- ✅ Cálculos matemáticos precisos y objetivos
- ✅ Análisis de tendencias basado en regresión lineal
- ✅ Métricas de volatilidad y riesgo
- ✅ Disponible 24/7 sin limitaciones de cuota

**Para análisis con IA:** Intente nuevamente en unos minutos cuando se restablezca la cuota.
"""
                
                return basic_report + fallback_note
            else:
                st.warning(f"⚠️ Error de IA. Usando análisis estadístico como alternativa.")
                return generate_basic_analysis_report(data_clean, ticker, ticker_description)
...
def generate_analysis_report(data: pd.DataFrame, ticker: str, ticker_description: str = "") -> str:
    """
    Función principal para generar análisis. Usa la versión optimizada por defecto.
    """
    return generate_analysis_report_optimized(data, ticker, ticker_description)
