...
def descargar_y_procesar_datos_economicos():
    """Descarga y procesa todos los datos desde las APIs oficiales del Ministerio de Economía"""
    
    with st.spinner("🔄 Descargando y procesando datos económicos del Ministerio de Economía..."):
        try:
...
def obtener_series_disponibles_economicas(datos):
    """Obtiene las series disponibles con metadatos del Ministerio de Economía"""
    if datos is None:
        return pd.DataFrame()
...
def obtener_valores_serie_economica(serie_id, datos):
    """Obtiene los valores históricos de una serie específica del Ministerio de Economía"""
    if datos is None:
        return pd.DataFrame()
...
def crear_grafico_serie_economica(valores_df, titulo, unidades):
    """Crea un gráfico interactivo para una serie temporal económica"""
    if valores_df.empty:
        return None
