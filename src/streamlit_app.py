...
def optimizar_portafolio(activos, valor_total, n_simulaciones=10000, tasa_libre_riesgo=0.0, comision_rebalanceo=0.01, sharpe_objetivo=None):
    """
    ...
    Args:
        ...
        sharpe_objetivo (float): Sharpe Ratio objetivo (opcional)
    ...
    try:
        ...
        # Seleccionar cartera según el Sharpe objetivo
        if sharpe_objetivo is not None:
            # Encontrar la cartera más cercana al Sharpe objetivo
            mejor_cartera = min(simulaciones, key=lambda x: abs(x['sharpe'] - sharpe_objetivo))
            mejor_sharpe = mejor_cartera['sharpe']
            resultados['metricas']['objetivo_alcanzado'] = abs(mejor_sharpe - sharpe_objetivo) < 0.05  # ±5% de tolerancia
        else:
            # Si no hay Sharpe objetivo, tomar la mejor Sharpe
            ...
            if st.form_submit_button("🚀 Ejecutar Optimización", use_container_width=True):
                ...
                with st.spinner("Optimizando portafolio..."):
                    resultados = optimizar_portafolio(
                        ...
                        sharpe_objetivo=st.number_input(
                            "Sharpe Ratio Objetivo:",
                            min_value=0.0,
                            max_value=5.0,
                            value=1.0,
                            step=0.1,
                            help="Sharpe Ratio objetivo para la optimización del portafolio"
                        )
                    )
...
def obtener_precios_historicos(simbolos, dias=252):
    ...
    try:
        for simbolo in simbolos:
            url = f"https://api.invertironline.com/api/v2/mercado/{simbolo}/historico"
            params = {
                'dias': dias,
                'tipo': 'Cierre'
            }
        ...
        st.error(f"Error al obtener precios históricos: {str(e)}")
        ...
        if response.status_code == 200:
            ...
            if portafolio:
                ...
                for activo in portafolio['activos']:
                    if activo['simbolo'] in precios_historicos:
                        precios = precios_historicos[activo['simbolo']]
                        if len(precios) > 1:
                            retornos = [(precios[i] - precios[i-1]) / precios[i-1] 
                                        for i in range(1, len(precios))]
                            activo['retornos_historicos'] = retornos
