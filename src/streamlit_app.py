...
def main():
    """Función principal que maneja la navegación y autenticación"""
    # Inicializar session state
    init_session_state()
    ...
    with st.sidebar:
        ...
        if not st.session_state.authenticated:
            ...
            if st.session_state.clientes:
                ...
                opciones_clientes = ["Seleccione un cliente..."]
                for cliente in st.session_state.clientes:
                    nombre = cliente.get('apellidoYNombre', cliente.get('nombre', 'Sin nombre'))
                    numero = cliente.get('numeroCliente', cliente.get('id', 'Sin ID'))
                    opciones_clientes.append(f"{nombre} ({numero})")
                ...
                if cliente_seleccionado_idx > 0:
                    st.session_state.cliente_seleccionado = st.session_state.clientes[cliente_seleccionado_idx - 1]
                    
                    cliente = st.session_state.cliente_seleccionado
                    st.success(f"Cliente seleccionado: {cliente.get('apellidoYNombre', 'Sin nombre')}")
    ...
    if not st.session_state.authenticated:
        ...
        st.markdown("""
        # 📊 IOL Portfolio Analyzer
        
        ### Bienvenido al Analizador de Portafolios de Invertir Online
        
        Esta aplicación le permite:
        - 📈 Analizar portafolios de clientes
        - 📊 Visualizar datos de volumen y precios
        - 🎯 Optimizar distribución de activos
        - 👤 Evaluar perfiles de riesgo
        - 🟢 Analizar opciones argentinas
        
        **Para comenzar, ingrese sus credenciales en el panel lateral.**
        """)
        ...
        with st.expander("ℹ️ Información de la Aplicación"):
            ...
            **Características principales:**
            ...
            - **Autenticación segura** con la API de IOL
            - **Análisis completo de portafolios** con métricas avanzadas
            - **Visualizaciones interactivas** usando Plotly
            - **Análisis de volumen** y datos técnicos
            - **Optimización de portafolios** con cálculo de VaR
            - **Panel de opciones** con modelos Black-Scholes y Binomial
            
            **Requisitos:**
            - Cuenta de asesor en Invertir Online
            - Credenciales válidas de la API
            """)
    
    else:
        # Usuario autenticado - mostrar contenido principal
        if st.session_state.cliente_seleccionado:
            # Mostrar análisis de portafolio
            ...
            Para continuar con el análisis, por favor seleccione un cliente en el panel lateral.
            """)
            ...
            if st.session_state.clientes:
                ...
                for i, cliente in enumerate(st.session_state.clientes[:10]):  # Mostrar solo los primeros 10
                    nombre = cliente.get('apellidoYNombre', cliente.get('nombre', 'Sin nombre'))
                    numero = cliente.get('numeroCliente', cliente.get('id', 'Sin ID'))
                    
                    if st.button(f"📋 {nombre} ({numero})", key=f"cliente_btn_{i}"):
                        st.session_state.cliente_seleccionado = cliente
                        st.rerun()
                
                if len(st.session_state.clientes) > 10:
                    st.info(f"Se muestran los primeros 10 clientes de {len(st.session_state.clientes)} disponibles. Use el selector en el panel lateral para ver todos.")
