...
class manager:
    def __init__(self, rics, notional, data):
        ...
        self.risk_free_rate = 0.40  # Tasa libre de riesgo anual argentina

    def load_intraday_timeseries(self, ticker):
        """Carga series de tiempo desde datos proporcionados"""
        return self.data[ticker] if ticker in self.data else None

    def synchronise_timeseries(self):
        """Sincroniza todas las series de tiempo"""
        dic_timeseries = {}
        for ric in self.rics:
            ts = self.load_intraday_timeseries(ric)
            if ts is not None:
                dic_timeseries[ric] = ts
        self.timeseries = dic_timeseries

    def compute_covariance(self):
        """Computa matriz de covarianza y retornos medios"""
        self.synchronise_timeseries()
        
        if not self.timeseries:
            raise ValueError("No se pudieron cargar series de tiempo")
        ...
        for ric in self.rics:
            if ric in self.timeseries:
                ...
                if isinstance(prices, pd.Series):
                    ...
                    # Si es un DataFrame, usar la columna 'Close' o similar
                    if isinstance(prices, pd.DataFrame):
                        if 'Close' in prices.columns:
                            ...
                            price_cols = ['close', 'price', 'ultimo', 'ultimoPrecio']
                            price_col = None
                            ...
                            if price_col:
                                ...
                                continue
                        returns_matrix[ric] = np.log(price_series / price_series.shift(1)).dropna()
        ...
        # Remover NaN y alinear datos
        self.returns = self.returns.dropna()
        
        if self.returns.empty:
            raise ValueError("No hay datos suficientes después de limpiar")
    ...
    def compute_portfolio(self, portfolio_type=None, target_return=None):
        """Computa portafolio optimizado según estrategia especificada"""
        ...
        # Filtrar activos que están realmente disponibles
        available_assets = list(self.returns.columns)
        if len(available_assets) != n_assets:
            st.warning(f"Solo {len(available_assets)} de {n_assets} activos están disponibles")
            n_assets = len(available_assets)
            bounds = tuple((0, 1) for _ in range(n_assets))
        
        # Función objetivo base para minimizar varianza
        def portfolio_variance_func(weights):
            return portfolio_variance(weights, self.cov_matrix.values)
        
        # Función para maximizar Sharpe ratio (minimizar -Sharpe)
        def neg_sharpe_ratio(weights):
            port_ret = np.sum(self.mean_returns.values * weights)
            port_vol = np.sqrt(portfolio_variance(weights, self.cov_matrix.values))
            if port_vol == 0:
                return np.inf
            return -(port_ret - self.risk_free_rate) / port_vol
        
        # Restricciones según estrategia
        if portfolio_type == 'min-variance-l1':
            ...
            objective = portfolio_variance_func
            ...
            return self._create_output(weights, available_assets)
            
        elif portfolio_type == 'long-only':
            # Optimización long-only estándar (minimizar varianza)
            ...
            if target_return is not None:
                ...
                constraints = [
                    {'type': 'eq', 'fun': lambda x: np.sum(x) - 1},
                    {'type': 'eq', 'fun': lambda x: np.sum(self.mean_returns.values * x) - target_return}
                ]
                objective = portfolio_variance_func
            else:
        ...
        try:
            ...
            if result.success:
                return self._create_output(result.x, available_assets)
            else:
                st.warning(f"Optimización no convergió para {portfolio_type}. Usando pesos iguales.")
                weights = np.ones(n_assets) / n_assets
                return self._create_output(weights, available_assets)
    ...
    def _create_output(self, weights, asset_names):
        ...
        port_output.asset_names = asset_names
        port_output.expected_return = port_ret
...
class output:
    ...
    def plot_histogram_streamlit(self, title="Distribución de Retornos"):
        ...
        fig = go.Figure(data=[go.Histogram(
            x=self.returns,
            nbinsx=min(50, max(10, len(self.returns)//10)),
        ...
        if len(self.returns) > 30:
            ...
            y_norm = stats.norm.pdf(x_norm, self.mean_daily, self.volatility_daily)
            # Escalar para que coincida con el histograma
            y_norm = y_norm * len(self.returns) * (self.returns.max() - self.returns.min()) / 50
        ...
        title_text = f"{title}<br>" + \
                    f"<sub>Sharpe: {self.sharpe_ratio:.3f} | " + \
                    f"Skewness: {self.skewness:.3f} | " + \
                    f"Kurtosis: {self.kurtosis:.3f} | " + \
                    f"JB p-value: {self.p_value:.3f}</sub>"
...
def compute_efficient_frontier(rics, notional, target_return, include_min_variance, data):
    ...
    try:
        # Etiquetas de portafolios especiales    
        labels = {
            'min-variance-l1': 'Min Variance L1',
            'min-variance-l2': 'Min Variance L2',
            'equi-weight': 'Pesos Iguales',
            'long-only': 'Solo Largos',
            'markowitz': 'Markowitz Sharpe',
            'markowitz-target': 'Markowitz Target'
        }
        ...
        # Computar vectores de retornos y volatilidades para portafolios Markowitz
        min_returns = np.min(port_mgr.mean_returns.values)
        max_returns = np.max(port_mgr.mean_returns.values)
        
        # Crear rango de retornos objetivo más conservador
        return_range = max_returns - min_returns
        target_returns = min_returns + np.linspace(0.1, 0.9, 30) * return_range
        ...
        for i, ret in enumerate(target_returns):
            try:
                progress_container.text(f"Calculando frontera eficiente: {i+1}/{len(target_returns)}")
                port = port_mgr.compute_portfolio('markowitz', ret)
                if port and port.volatility_annual > 0:
                    volatilities.append(port.volatility_annual)
                    valid_returns.append(port.return_annual)
            except:
                continue
        ...
        strategies = ['min-variance-l1', 'min-variance-l2', 'equi-weight', 'long-only', 'markowitz']
        
        for strategy in strategies:
            try:
                if strategy == 'markowitz':
                    portfolios[strategy] = port_mgr.compute_portfolio(strategy)
                else:
                    portfolios[strategy] = port_mgr.compute_portfolio(strategy)
            except Exception as e:
                st.warning(f"No se pudo calcular {strategy}: {str(e)}")
                portfolios[strategy] = None
...
class PortfolioManager:
    ...
    def load_data(self):
        ...
        try:
            ...
            if mean_returns is not None and cov_matrix is not None and df_precios is not None:
                ...
                data_dict = {}
                for col in df_precios.columns:
                    data_dict[col] = df_precios[col]
                
                self.manager = manager(list(df_precios.columns), self.notional, data_dict)
    ...
    def compute_portfolio(self, strategy='markowitz', target_return=None):
        ...
        try:
            if self.manager:
                ...
                portfolio_output.asset_names = list(self.returns.columns)
    ...
    def compute_efficient_frontier(self, target_return=0.08, include_min_variance=True):
        ...
        try:
            # Preparar datos para compute_efficient_frontier
            data_dict = {}
            for col in self.prices.columns:
                data_dict[col] = self.prices[col]
            
            portfolios, returns, volatilities = compute_efficient_frontier(
                list(self.prices.columns), self.notional, target_return, include_min_variance, 
                data_dict
            )
