        ...
        if st.button("🔍 Analizar Operaciones Reales", type="primary"):
            if id_cliente:
                ...
                if operaciones:
                    ...
                    # Agrupar por símbolo
                    analisis_simbolo = df_ops.groupby('simbolo').agg({
                        'tipo': 'count',
                        'cantidadOperada': 'sum',
                        'montoOperado': 'sum'
                    }).round(2)
                    
                    analisis_simbolo.columns = ['Total Operaciones', 'Cantidad Total', 'Monto Total']
                    analisis_simbolo = analisis_simbolo.sort_values('Monto Total', ascending=False)
                    
                    st.dataframe(analisis_simbolo, use_container_width=True)
                    
                else:
                    st.warning("⚠️ No se encontraron operaciones en el período especificado")
            else:
                st.error("❌ Debe seleccionar un cliente para analizar operaciones")
